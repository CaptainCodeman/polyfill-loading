...
WebPageTest results show fast load time and the app code executing before the polyfills have loaded. Using the blocking technique would force the execution of those to _after_ the polyfill script, in this case delaying them by at least 760ms (some requests take 2 seconds). Instead, the page is completely loaded, interactive and idle in about 1.4 seconds on a Moto G4, showing that the need to load polyfills for _some_ browsers hasn't inpacted potential performance.
![no polyfills web-page-test](images/no-polyfills-web-page-test.png)

Here are the requests showing that the app code is used _before_ the page is rendered and the polyfill code doesn't hold that up for a fully-supported browser.
![no polyfills web-page-test requests](images/no-polyfills-web-page-test-requests.png)
